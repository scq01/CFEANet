...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        ead4 = self.ead4(edge_info, x_g_3, out5)  # 128 16 16

        d4 = self.up4(d5, ead4)  # 256 32 32
        out4 = self.out4(d4)  # 1 32 32
        ead3 = self.ead3(edge_info, x_g_2, out4)  # 128 32 32

        d3 = self.up3(d4, ead3)  # 128 64 64
        out3 = self.out3(d3)  # 1 64 64
        ead2 = self.ead2(edge_info, x_g_1, out3)  # 128 64 64

        d2 = self.up2(d3, ead2)  # 128 128 128
        out2 = self.out2(d2)  # 1 128 128
        ead1 = self.ead1(edge_info, x1, out2)  # 64 128 128
